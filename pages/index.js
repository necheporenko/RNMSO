...
class Home extends React.Component {
  ...
  changeConcert = (id, index) => {
    this.setState({ currentConcert: id })
    this.concertSlider.slickGoTo(index);
  }


  render() {
    const { t, videos, news, concerts, language } = this.props;
    const { currentConcert } = this.state;
    ...
    return (
      <Layout title={t("OrchestraName")}>
                ...
                <div className="events__calendar">
                  <ul className="events__date">
                    {concerts.slice(-5).map((concert, index) => (
                      <li key={concert.id} onClick={() => this.changeConcert(concert.id, index)}>
                        <a className={currentConcert === index ? 'no-link no-link--active' : 'no-link'} style={{ cursor: 'pointer' }}>
        ...
        <section className="s-video" style={{ paddingTop: '30px' }}>
          <div className="container">
            <div className="row">
